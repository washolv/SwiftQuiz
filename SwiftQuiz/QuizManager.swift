...
class QuizManager{
    init(){
        self.quiz = Quiz(question: "", options: [], correctedAnwser: "")
        self._totalAnwsers = 0
        self._totalCorrectedAnwsers=0
    }
    ...
    private var _totalAnwsers = 0
    private var _totalCorrectedAnwsers=0
    ...
    var totalAnwsers: Int{
        return _totalAnwsers;
    }
    var totalCorrectAnwsers: Int{
        return _totalCorrectedAnwsers
    }
    func refreshQuiz(){
        let randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[randomIndex]
        quiz = Quiz(question: quizData.question, options: quizData.options, correctedAnwser: quizData.correctAnswer)
    }
    
    func validateAnwser(index: Int){
        _totalAnwsers += 1
        if quiz.validateOption(index){
            _totalCorrectedAnwsers += 1
        }
